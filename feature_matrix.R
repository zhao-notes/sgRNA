...
structure <- read.delim("MFE.txt", header=TRUE, sep="\t", stringsAsFactors = FALSE) # minimum free energy from ViennaRNA
nuc <- read.delim("nuc.count.txt", header=TRUE, sep="\t", stringsAsFactors = FALSE) # Tm file 
score <- read.delim("score.txt", header=TRUE, sep="\t", stringsAsFactors = FALSE) # sgRNA cutting efficiency score file
...
onehot.dep1 <- read.delim("dep1.txt", header=T, sep=",")
onehot.dep2 <- read.delim("dep2.txt", header=T, sep=",")
...
write.table(raw, "raw.matrix.txt", quote=F, row.names=F, sep="\t")



######## Calculate QCT features
# Read the ecoli_data.csv file
ecoli_data <- read.csv("ecoli_data.csv", header = TRUE, stringsAsFactors = FALSE)

# Transform the data
ecoli_data_transformed <- ecoli_data %>%
...
colnames(ecoli_data_transformed)[2:21] <- paste0("p", 1:20)

# Drop 'Quality' column
ecoli_data_transformed <- dplyr::select(ecoli_data_transformed, -Quality)

# Save the transformed data to sgRNA.sequence.txt
write.table(ecoli_data_transformed, "sgRNA.sequence.txt", quote = FALSE, row.names = FALSE, sep = " ")



# Monomer QCT
# Read sgRNA sequence data
seq <- read.delim("sgRNA.sequence.txt", header=T, sep=" ", stringsAsFactors = F)
...
# Basepair QCT
# Read sgRNA sequence data
seq <- read.delim("sgRNA.sequence.txt", header = TRUE, sep = " ", stringsAsFactors = FALSE)
...
write.csv(combined_qct, "quantum.melt.csv", row.names = FALSE)
write.table(combined_qct, "quantum.melt.txt", quote = FALSE, row.names = FALSE, sep = " ")
...
tensor <- read.csv("quantum.melt.csv", header = TRUE, stringsAsFactors = FALSE)
tensor[is.na(tensor)] <- 0

raw.tensor <- inner_join(raw, tensor, by="sgRNAID")

write.table(raw.tensor, "final_quantum.txt", quote=F, row.names=F, sep="\t")
write.csv(raw.tensor, "final_quantum.csv", row.names = FALSE)
